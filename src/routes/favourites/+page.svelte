<script lang="ts">
    ...
    import ShuffleModal from '$lib/components/modals/ShuffleModal.svelte'
    ...
    import { loadingStates, modals } from '$lib/stores/app.js'
    import { activeGridViewSideBarItem, cache } from '$lib/stores/pages.js'
    import type { Api } from '$lib/types/api.js'
    import type { App } from '$lib/types/app.js'

    import { onDestroy, onMount } from 'svelte'

    const { data } = $props()

    let mediaData = $derived<Api.MediaItem[]>($cache['favourites'].media)
    ...
    onMount(() => {
        ...
        if (data.success && Array.isArray(data.data)) {
            ...
            $cache['favourites'].media = [...$cache['favourites'].media]
            $activeGridViewSideBarItem = $cache['favourites'].media[0]
