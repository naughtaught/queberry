...
export const load: PageLoad = ({ url }) => {
    ...
    if (navigating.type === 'popstate' || url.pathname === '/video' || navigating.from?.url.pathname === '/video')
        return {
            success: true,
            data: currentCache['favourites'].media,
        }
    ...
    try {
        const dataPromise = (async () => {
            ...
            currentCache['favourites'].media = [...response.data]
            currentCache['favourites'].paginationPage = 0
            currentCache['favourites'].lastUpdated = Date.now()
            currentCache['favourites'].filters = current
