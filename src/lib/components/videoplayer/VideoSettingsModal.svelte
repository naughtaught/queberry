<script lang="ts">
    ...
    import { sessionSettings, SPEAKER_LAYOUTS_WITH_CENTER_SPEAKER, videoProperties } from '$lib/stores/video'

    let { bottom, left, currentModal = $bindable() } = $props()

    let isShaderMenuOpen = $state(false)

    const emitFunc = async (emit: string, value: number | string): Promise<App.Response> => {
        try {
...
<div
    ...
    <div class="flex flex-col gap-y-3">
        ...
        {#if SPEAKER_LAYOUTS_WITH_CENTER_SPEAKER.includes($videoProperties.audioChannel)}
            ...
            <div class="flex items-center justify-center gap-3">
                <button
                    ...
                    class="{$sessionSettings?.subtitlePos && $sessionSettings?.subtitlePos >= 100
                        ...
                        : 'bg-gray-800 text-gray-300 transition-colors duration-200 hover:cursor-pointer hover:bg-primaryColor'} flex items-center justify-center rounded-md px-3 py-1 text-sm"
                    onclick={async () => {
                        ...
                        if (result.success) $sessionSettings.subtitlePos = result.data.value
                    }}>
                    <MinusIcon />
                </button>
        ...
        {#if $videoProperties.availableShaders.length >= 1}
            <button
                class="float-right mt-5 flex w-full items-center justify-end text-right text-xs text-white transition-colors hover:text-primaryColor"
                onclick={() => {
        ...
        <div class="ml-5 flex flex-col">
            {#each $videoProperties.availableShaders as shader (shader.filename)}
                <button
                    ...
                    type="button"
                    class="flex w-full flex-col items-start gap-y-1 rounded-md px-1 py-1 transition-colors duration-200 hover:bg-primaryColor">
                    <div class="flex w-full items-center gap-x-1">
                        <span
                            class="flex h-3 w-3 shrink-0 items-center justify-center rounded-full border-2 {$videoProperties.activeShaders.some(
                                (x: string) => x === shader.filename,
                            )
