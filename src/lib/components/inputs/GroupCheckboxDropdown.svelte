<script lang="ts">
    ...
    let { dataArr, name, includeData = $bindable([]), searchEnabled = true } = $props()
    ...
    <div class="relative z-30 flex min-h-8.5 w-full flex-col" class:z-30={isOpen} class:z-10={!isOpen}>
        <button
            ...
            <span class="pointer-events-none min-w-0 flex-1 truncate">
                {#each includeData as item, i (i)}
                    <span>{item.trim()}{i < includeData.length - 1 ? ', ' : ''}</span>
                {/each}
            </span>
        ...
        {#if isOpen}
            <div
                ...
                {#if searchEnabled}
                    <div class="relative mx-auto my-1 flex w-[98%]">
                        <input class="w-full px-2 py-1" placeholder="Search..." bind:value={search} type="text" />
                        {#if search}
                            <button
                                type="button"
