...
pub struct RateLimiter {
    ...
    windows: Arc<DashMap<String, u64>>,
...
impl RateLimiter {
    ...
    pub fn with_window_seconds(mut self, seconds: u64) -> Self {
        self.default_window_seconds = seconds;
        self
    }
    ...
    pub fn set_window(&self, plugin_id: &str, window_seconds: u64) {
        self.windows.insert(plugin_id.to_string(), window_seconds);
    }
    ...
    pub fn check_limit(&self, plugin_id: &str) -> Result<(), AppError> {
        ...
        if entry.0.len() >= limit {
            return Err(AppError::RateLimit(format!(
                ...
                plugin_id, limit, window_seconds
            )));
        }
    ...
    pub fn cleanup_memory(&self) {
        let now = Instant::now();
        let max_idle_time = Duration::from_secs(24 * 3600);
