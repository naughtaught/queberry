...
pub fn set_audio_channel(mpv: &Arc<Mutex<Mpv>>, audio_channel: &str) -> Result<()> {
    let normalized_channel = normalize_audio_channel(audio_channel)?;

    let (current_position, current_volume) = {
        let guard = mpv
            .lock()
            .map_err(|e| AppError::Runtime(format!("Failed to lock MPV mutex: {}", e)))?;
        (
            guard.get_property::<f64>("time-pos").ok(),
            guard.get_property::<i64>("volume").unwrap_or(100),
        )
    };

    {
        ...
        let _ = guard.command("no-osd", &["af", "remove", "loudnorm"]);

        guard
            .set_property("audio-channels", normalized_channel)
            .map_err(|e| {
        ...
        if normalized_channel == "2.0" || normalized_channel == "2.1" {
            if let Err(e) = guard.command("no-osd", &["af", "add", "loudnorm=I=-16:TP=-1.5:LRA=11"])
            {
                log::debug!("Could not add loudnorm filter: {}", e);
            }
        }

        let _ = guard.set_property("volume", current_volume);
        let _ = guard.set_property("mute", false);

        if current_position.is_some() {
            let _ = guard.command("no-osd", &["seek", "0.001", "relative"]);
        }
    }
    ...
    if let Some(pos) = current_position {
        ...
        let guard = mpv
            .lock()
            .map_err(|e| AppError::Runtime(format!("Failed to lock MPV mutex: {}", e)))?;
        let _ = guard.command("no-osd", &["seek", &format!("{}", pos), "absolute"]);
    }

    Ok(())
}

pub fn set_audio_channel_during_init(mpv: &Mpv, audio_channel: &str) -> Result<()> {
    let normalized_channel = normalize_audio_channel(audio_channel)?;
...
fn normalize_audio_channel(audio_channel: &str) -> Result<&'static str> {
    match audio_channel.to_lowercase().as_str() {
        "7.1" => Ok("7.1"),
        "6.1" => Ok("6.1"),
        "6.0" => Ok("6.0"),
        "5.1" => Ok("5.1"),
