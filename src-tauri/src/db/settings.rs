    ...
    fn query_settings_from_db(&self, user_id: i32) -> Result<UserSettings, AppError> {
        ...
        match self.db.query_row(sql, [user_id], |row| {
            Ok(UserSettings {
                ...
                audio_channel: row.get(12)?,
                default_resolver: row.get(13)?,
                file_size_limit: row.get(14)?,
                disabled_plugins: row.get(15)?,
                season_completion_required: row.get(16)?,
                enable_user_ratings: row.get(17)?,
                open_app_fullscreen: row.get(18)?,
                update_notification: row.get(19)?,
                onscreen_keyboard_enabled: row.get(20)?,
                excluded_resolutions: row.get(21)?,
                excluded_video_formats: row.get(22)?,
                excluded_audio_options: row.get(23)?,
                excluded_sources: row.get(24)?,
                indexer_sort_criteria: row.get(25)?,
                download_rate_limit: row.get(26)?,
                screensaver_timeout: row.get(27)?,
            })
        }) {
